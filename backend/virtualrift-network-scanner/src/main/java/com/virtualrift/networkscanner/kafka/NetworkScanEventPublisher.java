...
@Service
public class NetworkScanEventPublisher {

    private static final Logger log = LoggerFactory.getLogger(NetworkScanEventPublisher.class);
    ...
    public NetworkScanEventPublisher(KafkaTemplate<String, ScanCompletedEvent> completedKafkaTemplate,
    ...
    public void publishCompleted(ScanCompletedEvent event) {
        completedKafkaTemplate.send(SCAN_COMPLETED_TOPIC, event.scanId().toString(), event)
                .whenComplete((result, ex) -> {
                    if (ex == null) {
                        log.info("Published scan.completed event for NETWORK scanId: {}", event.scanId());
                    } else {
                        log.error("Failed to publish scan.completed event for NETWORK scanId: {}", event.scanId(), ex);
                    }
                });
    }
    ...
    public void publishFailed(ScanFailedEvent event) {
        failedKafkaTemplate.send(SCAN_FAILED_TOPIC, event.scanId().toString(), event)
                .whenComplete((result, ex) -> {
                    if (ex == null) {
                        log.info("Published scan.failed event for NETWORK scanId: {}", event.scanId());
                    } else {
                        log.error("Failed to publish scan.failed event for NETWORK scanId: {}", event.scanId(), ex);
                    }
                });
    }
}
