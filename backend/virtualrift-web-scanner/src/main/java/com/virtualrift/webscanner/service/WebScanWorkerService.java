...
@Service
public class WebScanWorkerService {

    private static final String ERROR_CODE_INVALID_TARGET = "WEB_INVALID_TARGET";
    private static final String ERROR_CODE_PROCESSING_FAILED = "WEB_PROCESSING_FAILED";

    private final WebScanEngine engine;
    private final WebScanEventPublisher publisher;

    public WebScanWorkerService(WebScanEngine engine, WebScanEventPublisher publisher) {
        this.engine = engine;
    ...
    private boolean isWebScan(ScanRequestedEvent event) {
        return event != null && ScanType.WEB.name().equalsIgnoreCase(event.scanType());
    }
